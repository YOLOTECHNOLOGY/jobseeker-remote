...
const JobsCard = ({
  ...
  const card = (data) => {
    return data?.map((item, index) => {
      const { name, industry } = item.company || {}
      const {
        job_title: jobTitle,
      ...
      const { avatar, full_name: fullName, last_active_at: lastActiveAt } = item.recruiter || {}
      const workJobTitle = item.recruiter?.work_experience?.job_title
      const same = isSameDay(item.created_at, data[index - 1]?.created_at)

      const tagsList = [jobType, xpLvl, degree].filter(Boolean)

      return (
        <div key={`${item.id}`}>
          {!same && item.created_at && (
            <p className={styles.time}>{transDate(item.created_at?.substr(0, 10))}</p>
          )}
          <div className={`${styles.jobCard}`}>
            ...
            <div className={styles.name}>
              {status === 'closed' ? (
                <a onClick={(e) => e.preventDefault()}>{jobTitle}</a>
              ) : (
  ...
  const interviewCard = (data) => {
    return data?.map((item, index) => {
      ...
      const { name, logo_url: logoUrl } = item.company || {}
      const {
        salary_range_value: salaryRangeValue,
      ...
      return (
        <div key={`${item.id}`} className={styles.interviewed}>
          {!same && item.interviewed_at && (
            <p className={styles.time}>{transDate(item.interviewed_at?.substr(0, 10))}</p>
          )}
          <div className={`${styles.jobCard}`}>
              ...
              <div className={styles.left}>
                <div className={styles.company}>
                  ...
                  <span
                    ...
                    style={{ color: jobseekerDisplayStatusObject[status] || '#136FD3' }}
                  >
                    {status}
                  </span>
                </div>
