...
import { fetchJobsForYouLogin,fetchJobsPreferences } from 'store/services/jobs/fetchJobsForYouLogin';
...
return (
   <>
     <p className={styles.tips}>Want more accurate matches?
         <span className={styles.started} onClick={getStarted}>Get Started</span>  
        </p>
      {
      jobs?.map( (item,index) =>{
         const {id:Id,
         ...
         return (
            <div className={styles.jobCard} key={`${Id}-${index}`} onClick={()=>goToJobDetail(jobUrl)}>
            ...
            <div className={styles.contact}>
               <div className={`${styles.avator}  ${transTime(recruiterLastActiveAt) ? styles.avator2 : '' }`}>
      ...
      <p className={styles.load}>
         {
         loading ?  'Loading~' :  current === totalPage ? 'No more' : ''
         }
      </p>
   </>
