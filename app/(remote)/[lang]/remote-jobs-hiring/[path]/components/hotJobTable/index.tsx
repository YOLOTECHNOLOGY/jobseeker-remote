...
const HotJobTable = async ({ lang }) => {
  ...
  const statusResult = await fetchQueryOnlineStatus(
    result?.data?.data?.jobs?.map((item) => item.recruiter.id),
    accessToken
  )
  ...
  const jobs = result?.data?.data?.jobs?.map((item, i) => ({
    ...
    function_job_title_id: item.function_job_title_id
  }))

  return jobs && jobs.length ? (
    <>
      <h3 className={styles.hotjobTitle}>
        <span>{lang.popularJobs}</span>
      </h3>
      ...
      <ChatDataProvider recruiterIds={jobs.map((job) => job?.recruiter?.id).join(',')}>
        ...
        <div className={styles.mobileContainer}>
          {jobs?.map((job) => {
            return (
              <div className={styles.jobContainer} key={job?.id}>
                <MobileCard {...job} cardType='hotjobs' />
              </div>
            )
          })}
        </div>
